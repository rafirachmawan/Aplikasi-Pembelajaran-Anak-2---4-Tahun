...
function FruitOptionButton({
  ...
  return (
    ...
    >
      ...
      >
        <Text style={styles.fruitOptionText}>{option.label}</Text>
      </Animated.View>
    </Pressable>
  );
}

export default function FruitsQuestion() {
  const { level } = useLocalSearchParams<{ level: string }>();
  const router = useRouter();

  const levelId = parseInt(level) || 1;
  const levelData = fruitsData.levels.find((l) => l.id === levelId);
    ...
    if (correct) {
      ...
      setTimeout(() => {
        if (currentQuestion < questions.length - 1) {
          ...
          saveLevelProgress('fruits', String(levelId), newScore, questions.length);
        }
      }, 1600);
    } else {
      setFeedback('wrong');
