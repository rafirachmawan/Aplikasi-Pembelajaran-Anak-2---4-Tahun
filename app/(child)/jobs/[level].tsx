...
function JobOptionButton({
  ...
  return (
    ...
    >
      ...
      >
        <Text style={styles.jobOptionText}>{option.label}</Text>
      </Animated.View>
    </Pressable>
  );
}

export default function JobsQuestion() {
  const { level } = useLocalSearchParams<{ level: string }>();
  const router = useRouter();

  const levelId = parseInt(level) || 1;
  const levelData = jobsData.levels.find((l) => l.id === levelId);
    ...
    if (correct) {
      ...
      setTimeout(() => {
        if (currentQuestion < questions.length - 1) {
          ...
          saveLevelProgress('jobs', String(levelId), newScore, questions.length);
        }
      }, 1600);
    } else {
      setFeedback('wrong');
