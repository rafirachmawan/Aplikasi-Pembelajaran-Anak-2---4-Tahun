...
function AnimalOptionButton({
  ...
  return (
    ...
    >
      ...
      >
        <Text style={styles.animalOptionText}>{option.label}</Text>
      </Animated.View>
    </Pressable>
  );
}

export default function AnimalsQuestion() {
  const { level } = useLocalSearchParams<{ level: string }>();
  const router = useRouter();

  const levelId = parseInt(level) || 1;
  const levelData = animalsData.levels.find((l) => l.id === levelId);
    ...
    if (correct) {
      ...
      setTimeout(() => {
        if (currentQuestion < questions.length - 1) {
          ...
          saveLevelProgress('animals', String(levelId), newScore, questions.length);
        }
      }, 1600);
    } else {
      setFeedback('wrong');
