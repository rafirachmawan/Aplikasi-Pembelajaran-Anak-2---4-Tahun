...
import bodypartsData from '@/assets/data/bodyparts.json';
...
function BodyPartOptionButton({
  option,
  onPress,
}: {
  option: BodyPartOption;
  onPress: () => void;
}) {
  ...
  return (
    ...
    >
      ...
      >
        <Text style={styles.bodyPartOptionText}>{option.label}</Text>
      </Animated.View>
    </Pressable>
  );
}

export default function BodyPartsQuestion() {
  const { level } = useLocalSearchParams<{ level: string }>();
  const router = useRouter();

  const levelId = parseInt(level) || 1;
  const levelData = bodypartsData.levels.find((l) => l.id === levelId);
    ...
    if (correct) {
      ...
      setTimeout(() => {
        if (currentQuestion < questions.length - 1) {
          ...
          saveLevelProgress('bodyparts', String(levelId), newScore, questions.length);
        }
      }, 1600);
    } else {
      setFeedback('wrong');
    ...
    return (
      <SafeAreaView style={styles.container}>
        <View style={styles.finishContainer}>
          ...
          <Text style={styles.finishTitle}>Kamu Super Pintar!</Text>
          ...
          <View style={styles.finishButtons}>
            <Pressable
              style={[styles.finishButton, { backgroundColor: '#00B894', borderBottomColor: '#00876C' }]}
              onPress={() => {
      ...
      {feedback === 'correct' && (
        <View style={styles.feedbackOverlay}>
          <View style={styles.feedbackCard}>
            ...
            <Text style={styles.feedbackText}>Hore! Jawabanmu Benar! 🎉</Text>
          </View>
        </View>
      )}
      ...
      {feedback === 'wrong' && (
        <View style={styles.feedbackOverlay}>
          <View style={[styles.feedbackCard, { backgroundColor: '#FFEAA7' }]}>
            <Text style={styles.feedbackEmoji}>💪😊</Text>
            <Text style={styles.feedbackTextWrong}>Ayo coba sekali lagi!</Text>
          </View>
        </View>
      )}
    </SafeAreaView>
